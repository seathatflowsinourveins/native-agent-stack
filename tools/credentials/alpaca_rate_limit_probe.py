#!/usr/bin/env python3
"""Read-only Alpaca trading-API rate-limit probe.

Sends exactly one GET /v2/account to the fixed live or paper host and reports the HTTP
status and the x-ratelimit-* response headers. That is Alpaca's own statement of the
per-account calls-per-minute limit (200 on the standard tier; 1000 on Elite or a
non-retail account), so no order is ever needed to measure it. The probe never reads
the response body, never follows a redirect, never prints account identifiers or
credentials, and can send no other request.

Live keys are never stored (docs/secret-storage.md, "Live keys"): --account live asks for
both values with hidden input in an interactive terminal, uses them for the one request
and discards them. It refuses --env-file and refuses to run without a terminal. Paper keys
may come from the store (the alpaca-paper entry) or be typed with --prompt.

    python3 tools/credentials/alpaca_rate_limit_probe.py --account live --out RESULT.json
    python3 tools/credentials/alpaca_rate_limit_probe.py --account paper --env-file "$PAPER_ENV_FILE"
"""
...
def read_env_file(path: Path, uid: int | None = None) -> tuple[str, str]:
    """Paper keys only: store rules plus a post-open identity check (no TOCTOU, no FIFO)."""
...
def prompt_credentials(prompt=getpass.getpass) -> tuple[str, str]:
    """Hidden input for a one-off probe; the values live only in this process."""
    return tuple(_check_token(n, prompt(f"  {n} (hidden): ")) for n in NAMES)  # type: ignore[return-value]
...
def interpret(limit: str | None) -> str:
    ...
    if value >= 1000:
        return f"{value} calls/min: higher tier (Elite or non-retail)"
    if value == 200:
        return "200 calls/min: standard tier"
    return f"{value} calls/min"


def probe(account: str, key_id: str, key_secret: str, *, opener=None, now=None) -> dict:
    if account not in HOSTS:
        raise Refused("account must be live or paper")
    opener = opener or urllib.request.build_opener(_NoRedirect())
    request = urllib.request.Request(HOSTS[account] + PATH, method="GET", headers={
        "APCA-API-KEY-ID": key_id, "APCA-API-SECRET-KEY": key_secret, "User-Agent": USER_AGENT,
        "Accept": "application/json"})
    result = {"account": account, "host": HOSTS[account].split("//", 1)[1], "request": f"GET {PATH}",
    ...
    return {**result, "http_status": status, "rate_limit_headers": limits,
            ...
            + ("" if status == 200 else f" (HTTP {status}: check the key and account type)")}
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--account", choices=sorted(HOSTS), required=True)
    source = parser.add_mutually_exclusive_group()
    source.add_argument("--env-file", type=Path, help="paper only: stored credential file")
    source.add_argument("--prompt", action="store_true", help="type both values with hidden input")
    parser.add_argument("--out", type=Path, help="also write the non-secret result JSON here (mode 0600)")
    args = parser.parse_args(argv)
    interactive = args.account == "live" or args.prompt
    try:
        if args.account == "live" and args.env_file:
            raise Refused("live keys are never read from a file; they are typed at the prompt")
        if interactive:
            if not (sys.stdin.isatty() and sys.stdout.isatty()):
                raise Refused("run this in an interactive terminal (live keys are typed, never piped)")
            warnings.simplefilter("error", getpass.GetPassWarning)  # never fall back to echoed input
            key_id, key_secret = prompt_credentials()
        else:
            key_id, key_secret = read_env_file(args.env_file or paper_store_file())
        result = probe(args.account, key_id, key_secret)
        del key_id, key_secret
    except Refused as error:
        print(f"refused: {error}", file=sys.stderr)
        return 2
    except getpass.GetPassWarning:
        print("refused: this terminal cannot hide input", file=sys.stderr)
        return 2
    except (KeyboardInterrupt, EOFError):
        print("\ncancelled; nothing sent", file=sys.stderr)
        return 130
