...
**Evidence boundary:** DuckDB and exchange_calendars have an existing, narrowly scoped native receipt. The other 34 entries are `source_review`. No new software, market-data subscription, provider request, model inference, database server or trading action was executed during this catalog review. Every `native_workflow` is explicitly **prospective and unexecuted**. Commands use upstream executables/SDKs with illustrative inputs; they are not claimed to be verbatim upstream tutorials or completed end-to-end workflows. The individual current versions are not a jointly resolved dependency lockfile.
