...
The subsequent [research-runtime receipt](../../blueprints/us-equities/research-runtime/receipt.json) records a fresh LEAN bundled simulation and successful Dagu `packet`/`order_table` preparation. A **standalone** native Claude Opus 5 report cited that evidence; its 14,583 total tokens matched native metrics and logs. That attempt's paired graph was allowance-blocked. After native sign-in, [fresh paired acceptance](../../adoption/paired/receipt.json) completed, followed by a separately observed [architecture review pair](../../blueprints/us-equities/architecture/receipt.json). These remain local manual research evidence, not a validated financial strategy or unattended execution service.
