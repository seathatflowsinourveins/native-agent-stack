...
Check hardware fit before choosing: `python3 scripts/hardware_profile.py`
(stdlib only, [`scripts/hardware_profile.py`](../scripts/hardware_profile.py))
measures this host and compares it with the capacity tiers in
[`hardware-profiles.json`](hardware-profiles.json) (local generation model,
semantic-RAG embedding, bounded-run concurrency). `semantic-rag` and any
local model serving need a tier that fits.

The pin columns count how many of a profile's components have a SHA-256 pin in
...
3. Run the nonmutating prerequisite report. It reports executable presence, platform compatibility and recipe references. It never logs in, edits client configuration, starts services, executes catalog commands or certifies functional acceptance. Its opt-in `--client-wiring` also parses fixed Claude Code and Codex configuration files and reports, as booleans, counts and a computed `complete` only, whether the selected token practice is wired into both clients; the exit code stays the prerequisite result (changed after `v2026.09.24.1`: that release's `adoption_status.py` has no `--client-wiring` and its manifest has no `token-efficiency` profile, so run that check from a default-branch clone).
...
# changed after v2026.09.24.1, so run it from a default-branch clone until the next release):
