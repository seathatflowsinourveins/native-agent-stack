...
| `token-efficiency` | Drafted, not accepted, and changed after `v2026.09.25.1`, whose manifest lacks it: the selected token practice (RTK, Context Mode, explicit-file Repomix, guarded Headroom and TOON, ccusage, QMD, MarkItDown, Serena, SocratiCode, ai-memory, MCPorter) wired into both native clients | all 14 (8 of 14 at `v2026.09.25.2`) | 6 of 14 | The [coverage check](../docs/token-efficiency-stack.md#coverage-check) for command presence and client wiring, then pinned-version evidence and one useful native call per tool in each client, recorded through a PR. The Linux bootstrap now pins all of it; the macOS bootstrap still pins only part -- install the rest through its recipes |
