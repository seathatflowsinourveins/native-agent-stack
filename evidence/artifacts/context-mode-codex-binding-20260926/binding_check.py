...
Changed after the retained runs, which used binding_check.original.py: a request's time bound now covers the
whole request, not each wait for the next message (deadline_control.py).
"""
...
class Server:
    ...
    def request(self, method: str, params: dict, seconds: float = 90) -> dict:
        """The answer to one request. ``seconds`` bounds the whole request, however many other messages arrive
        meanwhile. Past it, the request raises queue.Empty, or TimeoutError (an OSError) when messages kept
        arriving; main() records either and then closes the server."""
        self.next_id += 1
        ...
        while True:
            ...
            if remaining <= 0:
                raise TimeoutError(f"no answer to {method} within {seconds} s")
