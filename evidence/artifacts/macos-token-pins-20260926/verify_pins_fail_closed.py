...
Fail-closed copy of verify_pins.py (2026-09-26, GPT-6 verification of this change). That harness,
kept byte-identical for the runs it made, skipped headroom's requires_python check when the
`packaging` module was not importable and could still end in PASS. This copy needs `packaging`:
without it, it exits 2 before any network access or file write. It also turns every other check that
could silently not run into a failure: a malformed requires_python, a missing headroom sdist and a
missing markitdown py3-none-any wheel each record a MISMATCH. Its header records the Python and
`packaging` versions.
"""
...
try:
    ...
    print("verify_pins_fail_closed.py needs the packaging module to check headroom's requires_python; "
...
log(f"# Python {platform.python_version()}; packaging {packaging.__version__}")
for label, pins_file in (("macOS pins", PINS), ("Linux pins", LINUX_PINS)):
    log(f"# {label} checked: {pins_file.name} sha256 {hashlib.sha256(pins_file.read_bytes()).hexdigest()}")
log()
...
for tool_id, project in (("headroom", "headroom-ai"), ("markitdown", "markitdown")):
    ...
    if tool_id == "headroom":
        # Run 1 of this check asserted "the only macOS wheel is the pinned arm64 one" and "no
        # platform-independent artifact"; both failed against PyPI (an x86_64 macOS wheel and a
        # maturin sdist exist). The claim that matters for an Apple Silicon install is which wheel
        # an arm64 interpreter can select: an arm64 or universal2 macOS wheel, never x86_64.
        mac = [e["filename"] for e in files if "macosx" in e["filename"]]
        arm64_compatible = [name for name in mac if "arm64" in name or "universal2" in name]
        log(f"  macOS wheels listed: {mac}")
        log(f"  arm64-compatible macOS wheels (arm64 or universal2): {arm64_compatible}")
        check("headroom: the pinned wheel is the only arm64-compatible macOS wheel", arm64_compatible == [filename])
        check("headroom: no py3-none-any wheel", not any(e["filename"].endswith("-none-any.whl") for e in files))
        sdist = next((e for e in files if e["packagetype"] == "sdist"), None)
        check("headroom: PyPI lists an sdist", sdist is not None)
        if sdist is not None:
            sdist_data = download(sdist["url"], sdist["filename"])
            log(f"  downloaded {sdist['filename']}: {len(sdist_data)} bytes, sha256 {digests(sdist_data)[0]}")
            check("headroom: sdist re-hash == PyPI digest", digests(sdist_data)[0] == sdist["digests"]["sha256"])
            with tarfile.open(FETCH / sdist["filename"], "r:gz") as archive:
                root = sdist["filename"][: -len(".tar.gz")]
                pyproject_text = archive.extractfile(f"{root}/pyproject.toml").read().decode()
                rust_sources = sum(1 for name in archive.getnames() if name.endswith(".rs"))
            backend = next((line.strip() for line in pyproject_text.splitlines()
                            if line.strip().startswith("build-backend")), None)
            log(f"  sdist pyproject.toml: {backend}; {rust_sources} .rs files in the archive")
            check("headroom: the sdist is a maturin (Rust) build, not a pure-Python artifact",
                  backend == 'build-backend = "maturin"' and rust_sources > 0)
        ...
        try:
            admits = requires is not None and "3.13" in SpecifierSet(requires)
        except InvalidSpecifier:
            admits = False
        check("headroom: requires_python admits 3.13 (the bootstrap's --python 3.13)", admits)
    else:
        ...
        check("markitdown: PyPI lists a py3-none-any wheel", wheel is not None)
...
# What `uv tool install git+https://github.com/oraios/serena@<commit>` itself needs: git resolves
# that exact object from the upstream repository (a scratch bare repository, nothing else touched).
bare = FETCH / "serena.git"
run("git", "init", "--quiet", "--bare", str(bare))
run("git", "-C", str(bare), "fetch", "--quiet", "--depth=1", "https://github.com/oraios/serena.git", commit)
cat_file = subprocess.run(["git", "-C", str(bare), "cat-file", "-e", f"{commit}^{{commit}}"],
                          capture_output=True, text=True, check=False, stdin=subprocess.DEVNULL)
log(f"  git fetch --depth=1 https://github.com/oraios/serena.git {commit}; git cat-file -e {commit}^{{commit}} -> exit {cat_file.returncode}")
check("serena: git fetches the pinned commit by hash and cat-file accepts it", cat_file.returncode == 0)
