...
# ---- verifier extensions (read-only): service start times, bin symlink table, stat-only hook/config mtimes
ext = {}
svc = {}
for u in fp["user_services"]["running"]:
    rc, out = sh(f"systemctl --user show {u} -p ActiveEnterTimestamp -p MainPID -p NRestarts -p ExecMainStartTimestamp")
    svc[u] = dict(l.split("=", 1) for l in out.strip().splitlines() if "=" in l)
ext["service_start"] = svc
bind = pathlib.Path(f"{E}/bin")
tbl = {}
for p in sorted(bind.iterdir()):
    st = os.lstat(p)
    tbl[p.name] = {"target": os.readlink(p) if p.is_symlink() else None, "lstat_mtime_ns": st.st_mtime_ns}
ext["bin_table_sha256"] = sha(json.dumps({k: v["target"] for k, v in tbl.items()}, sort_keys=True))
ext["bin_entries"] = len(tbl)
ext["bin_max_lstat_mtime_utc"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(max(v["lstat_mtime_ns"] for v in tbl.values()) / 1e9))
ext["bin_mcporter"] = tbl.get("mcporter")
hk = {}
for p in (f"{HOME}/.claude/settings.json", f"{HOME}/.claude/settings.local.json", f"{HOME}/.codex/config.toml",
          f"{HOME}/.codex/hooks.json", f"{HOME}/.claude/plugins/installed_plugins.json",
          f"{HOME}/code/native-agent-stack/.claude/settings.json", f"{E}/config/mcporter.json", f"{E}/installed-versions.txt"):
    e = stat_entry(p)
    hk[p.replace(HOME, "~")] = (time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(e["mtime_ns"] / 1e9)) if e else None)
ext["hook_config_mtimes_utc"] = hk
units_dir = pathlib.Path(f"{HOME}/.config/systemd/user")
ext["systemd_user_dir_max_mtime_utc"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(max(os.lstat(os.path.join(dp, n)).st_mtime for dp, dn, fn in os.walk(units_dir) for n in dn + fn + ["."])))
outp2 = HERE / "fp" / f"{sys.argv[1]}.ext.json"
outp2.write_text(json.dumps(ext, indent=1, sort_keys=True).replace(HOME, "~") + "\n")
print(json.dumps({"bin_mcporter": ext["bin_mcporter"]["target"].replace(HOME, "~"), "bin_max_mtime": ext["bin_max_lstat_mtime_utc"],
                  "units_dir_max_mtime": ext["systemd_user_dir_max_mtime_utc"], "hooks": ext["hook_config_mtimes_utc"],
                  "svc_start": {k: v.get("ActiveEnterTimestamp") for k, v in svc.items()},
                  "svc_nrestarts": {k: v.get("NRestarts") for k, v in svc.items()}}, indent=1).replace(HOME, "~"))
