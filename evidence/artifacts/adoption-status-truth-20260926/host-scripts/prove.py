#!/usr/bin/env python3
"""u5 E2E proof: does scripts/adoption_status.py tell this host's truth, and are the G13 hygiene fixes live?
...
local_integration. Every observation comes from an upstream command or native API; this script only compares them:
  ...
  * ai-memory capture admission: `ai-memory --data-dir <the installed Codex hook's> hook --check-capture` (no spool,
    no server contact), which must report capture_mode "allowlist" with marker_present and admits_capture true;
...
It prints booleans, counts and ids, never a hook command, credential or environment value. A check whose native
observation is unavailable, or that has nothing it could contradict, is reported UNTESTED rather than passed. Exit 0
when every check passes, 1 when any fails, 3 when none fails but some are untested.

The Codex probes start a Codex runtime on this host's CODEX_HOME: like any Codex start, that writes Codex's own log
and state databases there, may run its plugin sync and may refresh its sign-in token. Nothing here edits a
configuration or repository file.
"""
...
def check(name: str, passed: bool | None, detail: str) -> None:
    """Record a check: True PASS, False FAIL, None UNTESTED (no native observation could contradict it)."""
    status = "UNTESTED" if passed is None else "PASS" if passed else "FAIL"
    RESULTS.append((name, status, detail))
    print(f"{status}  {name}: {show(detail)}", flush=True)
...
def hook_data_dir() -> str | None:
    """The --data-dir that the installed Codex ai-memory hook commands carry: the capture check must read the
    capture-mode file the hook reads, since without one ai-memory answers denylist mode, which admits everything."""
    try:
    ...
    for groups in hooks.values() if isinstance(hooks, dict) else []:
        for group in groups if isinstance(groups, list) else []:
            for handler in (group.get("hooks") or []) if isinstance(group, dict) else []:
                ...
                if not isinstance(command, str) or not runs_ai_memory_hook(command):
                    continue
                argv = shlex.split(command)
...
def check_capture(path: Path) -> dict:
    data_dir = hook_data_dir()
    if data_dir is None:
        return {}
    result = run(["ai-memory", "--data-dir", data_dir, "hook", "--event", "pre-tool-use", "--agent", "codex",
                  "--server-url", os.environ.get("AI_MEMORY_HOOK_URL", "http://127.0.0.1:49474"), "--check-capture"],
...
def enrolled(verdict: dict) -> bool:
    """Admitted because the marker was found under an allowlist, not because a default denylist admits anything."""
    return (verdict.get("capture_mode") == "allowlist" and verdict.get("marker_present") is True
            and verdict.get("admits_capture") is True)
...
def main() -> int:
    ...
    else:  # with no native blocker, other booleans may still decide it: nothing here can contradict the checker
        check("G9 complete is false while Codex misses the practice", None if fixed else False,
    ...
    check("G13 render.py's text cites the identified G1 cause (a text check)", "the cause is open" not in render
          and "G1" in render and 'instance="unscoped"' in render, "panels 10 and 19 name the unscoped series")
    try:
        ...
        series = float(unscoped[0]["value"][1]) if unscoped else 0.0
        reset_count = float(resets[0]["value"][1]) if resets else 0.0
        check("G13 the G1 cause render.py cites holds in Prometheus now", series > 0 and reset_count > 0,
              f"{series:.0f} series with instance=unscoped, {reset_count:.0f} counter resets in 1 h")
    except (OSError, ValueError, KeyError, IndexError) as error:
        check("G13 the G1 cause render.py cites holds in Prometheus now", None, f"Prometheus not queried ({error})")
    ...
    marked = [path for path in linked if (path / ".ai-memory.toml").is_file()]
    admitted = [path for path in marked if enrolled(check_capture(path))]
    print(f"      worktrees: {len(linked)} linked, {len(outside)} outside $HOME, {len(marked)} carry the marker, "
          f"{len(admitted)} of those admitted through it under allowlist mode")
    sample = next((path for path in outside if path not in marked), None)
    ...
    for path in (Path(item).resolve() for item in args.worktree):
        ...
        check(f"G13 enrolled worktree admits capture through its marker ({path.name})", enrolled(verdict),
              f"capture_mode {verdict.get('capture_mode')}, marker_present {verdict.get('marker_present')}, "
              f"admits_capture {verdict.get('admits_capture')}")
    ...
    untested = [name for name, status, _ in RESULTS if status == "UNTESTED"]
    print(f"\n{len(RESULTS) - len(failed) - len(untested)} of {len(RESULTS)} checks passed"
          + (f"; failed: {len(failed)}" if failed else "") + (f"; untested: {len(untested)}" if untested else ""))
    return 1 if failed else 3 if untested else 0
