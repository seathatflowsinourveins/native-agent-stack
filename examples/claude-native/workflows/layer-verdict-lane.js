export const meta = {
  ...
  phases: [
    ...
    { title: 'Re-check', detail: 'evidence lens + challenger lens re-check the revision, Opus high', model: 'opus' },
  ],
}
...
const refute = async (p, subject, round) => {
  const got = await parallel(LENSES.map((lens) => () =>
    agent(lensPrompt(p, subject, lens), { label: (round === 'revision' ? 'refute:recheck:' : 'refute:') + `${lens}:${p.catalog}/${p.layer_id}`, phase: round === 'revision' ? 'Re-check' : 'Refute', agentType: 'evidence-reviewer', model: 'opus', effort: 'high', schema: VOTE }).catch(() => null)))
