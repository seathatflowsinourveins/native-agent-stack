...
Each child also carries a `lanes` object read from its own transcript: tool calls,
...
position of the text a shell runs, so quoted text and heredoc bodies count only under
`sh -c`, `eval`, `ssh` or a shell heredoc, with loopback-only calls apart), the
...
and a ToolSearch load in the window of its result, so adjacent windows add up. It
groups by spawn path, agent type and anonymous session ordinal, keeps `blind-*`
