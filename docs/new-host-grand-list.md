...
Layers: 20 foundation, 12 trading. Winners: 66 layer-winner pairs (55 distinct components). Pairs accepted end to end: 22 on WSL2, 0 on macOS. Distinct components pinned behind upstream: 12. Pairs a new host still has to prove: 18 on WSL2, 60 on macOS (the `needs_host` lists in `catalogs/landscape/component-evidence-matrix.json`).
