...
Layers: 20 foundation, 12 trading. Winners: 66 layer-winner pairs (55 distinct components). Pairs accepted end to end: 33 on WSL2, 0 on macOS. Distinct components pinned behind upstream: 13. Pairs a new host still has to prove: 33 on WSL2, 66 on macOS (the `needs_host` lists in `catalogs/landscape/component-evidence-matrix.json`).
...
|  |  | `candidate:cli-cli` | unpinned | source_review | not_established, pinned | untested, pinned | — |  |
...
|  |  | `candidate:astral-sh-uv` | unpinned | synthetic | conditional, pinned | untested, pinned | — |  |
...
|  |  | `foundation-ai-memory` | v2.3.1 (behind v2.4.0) | native_proven | accepted, pinned | untested, pinned | foundation-cpu, recovery, macos-arm64-foundation |  |
