...
The [retained functional follow-up](../evidence/receipts/token-native-retained-functional-20260920.json) links twelve existing accepted operations to Context Hub, Headroom, MCP Inspector, promptfoo and Worktrunk. Headroom used a local guard around upstream APIs; promptfoo used a local echo provider. Worktrunk completed create/select/list/remove in a disposable Git repository with hooks disabled. Context Hub grew by 85 tokens on its retained comparison, and rejected compression retains original input. These outcomes remain visible.
