...
For requested harness capabilities, first inspect installed native help and current official feature documentation, then review maintained upstream implementations and relevant community alternatives. Resolve discoverable names and capabilities before asking the user. Compare alternatives against a concrete gap, record source pins and adoption reasons, and execute the selected native path through actual returned results. A dated exclusion or missing catalog row is not current availability evidence. This research is task-triggered, not a full-catalog startup ritual. The [native Ultracode recipe](../recipes/claude-native-ultracode.md) records the current dispatch, messaging and dashboard boundary.
...
Keep component pins in `manifests/stack.json`, scoped claims and hashes in `manifests/evidence.json`, and decisions in the appropriate catalog. Obtain independent review for substantive changes, run the affected checks, inspect the actual changed UI when applicable, and verify the accepted GitHub revision. A local result, CI fixture, another host and broker/model execution are distinct evidence scopes.
