...
GitHub has one toggle for both halves: "Allow GitHub Actions to create and approve pull requests" controls "whether GITHUB_TOKEN can create and approve pull requests" ([managing GitHub Actions settings](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/enabling-features-for-your-repository/managing-github-actions-settings-for-a-repository#preventing-github-actions-from-creating-or-approving-pull-requests)). OpenSSF's SCM best practices rate "Workflows Should Not Be Allowed To Approve Pull Requests" high ([source](https://best.openssf.org/SCM-BestPractices/github/repository/actions_can_approve_pull_requests.html)). A read-only `gh api repos/seathatflowsinourveins/native-agent-stack/actions/permissions/workflow` at 2026-09-25T06:34:07Z returned `{"default_workflow_permissions":"read","can_approve_pull_request_reviews":true}`.

- **Decision.** Keep the setting on, as the live test above decided, because turning it off also stops `propose`'s `gh pr create`. The risk is small today: ruleset 23739774 requires 0 approving reviews and has no bypass actors, so a workflow approval has nothing to bypass. The unused approve capability is now guarded by `tests/test_workflow_hardening.py`'s `NoWorkflowApprovesPullRequestsTests`, across every workflow:
  - `pull-requests: write` is granted only to `catalog-freshness.yml:propose` (top-level and job-level blocks, comments stripped). `tests/test_catalog_freshness_propose.py` still pins that job's exact scopes.
  - No workflow holds `actions: write`, the scope `POST /repos/{owner}/{repo}/actions/runs/{run_id}/approve` needs; approving a pending run stays a reviewer's act.
  - No non-comment line runs `gh pr review`, `--approve`, a `pulls/<n>/reviews` call, `APPROVE` or an auto-approve action.
  - `permissions: write-all`, any inline `permissions:` form and any `permissions:` key the block parser did not parse fail the test, so a grant cannot slip past it.
- **Overturn.** Before `required_approving_review_count` rises above 0, move `propose` to a GitHub App installation token (`actions/create-github-app-token`, pinned by SHA), then turn the setting off. Turning it off first breaks `gh pr create`. An App token also lets the bot PR's own `pull_request` checks run without the manual approval step ([GITHUB_TOKEN](https://docs.github.com/en/actions/concepts/security/github_token)).
