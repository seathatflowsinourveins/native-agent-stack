...
- **Report-only lanes:** the Monday `catalog-freshness` lane rebuilds the
  ...
  artifact, without writing back. `adoption-bootstrap` exercises the pinned
  bootstrap scripts on hosted runners. Scorecard and `security-scan`'s
  `zizmor-online` job upload SARIF to code scanning without failing on findings.
- **Security gates:** `supply-chain`'s `sbom-vuln` inventories the pinned rc5
  install with Syft and fails at Grype `--fail-on high` with a reviewed
...
- **Code scanning and updates:** CodeQL default setup is configured (closure
  ...
  security updates are on; Dependabot pip version updates stay off because a
  single-package bump cannot reproduce the hash-locked uv compiles.
- **Secret scanning coverage:** `secret-scan` fails on any finding. It uses
  reviewed, path- and key-scoped allowlists and skips files over 2 MB. The
  generated explorer HTML is no longer committed, so only the explorer blobs in
  older history are skipped; their sources are scanned. Its first hosted run
  timed out on full history before the size skip was added.
- **Releases:** pushing a `v*` tag runs `publish-catalog`, which archives the
  validated commit and attests the archive, an SPDX SBOM and the rebuilt
  explorer. A tag-only `release` job then creates an immutable GitHub Release
  with the attested archive and SBOM attached at creation and checks that the
  published release is immutable and carries the attested digests. Verify a
  file with `gh attestation verify` and a release asset with
  `gh release verify-asset`. No hosted tag run has been recorded yet.
...
- Dependabot (conditional) — The initial hosted scans succeeded and opened PRs #29 and #30. Their integrity failures, on unreviewed workflow hashes, were retained (docs/github-automation.md lines 56-60; catalogs/foundation/automation.json@92bb279 active_baseline.dependabot_observation and selected_upgrade_qualification.original_proposals). The weekly Monday 09:00 UTC timing and a nonempty minor/patch group remain unobserved (docs/github-automation.md line 61; catalogs/foundation/automation.json@92bb279 remaining_unverified[0]). It owns GitHub Actions references, plus (since 2026-09-22) one ignored pip fixture entry (.github/dependabot.yml, grype-known-cve-fixture, open-pull-requests-limit: 0) that keeps a deliberately vulnerable pin's alert visible without proposing an update. It is not activated for the real pip locks, because .github/requirements-ci.lock is not a name it discovers, and four of the five pinned CI binaries are curl-downloaded tarballs it cannot parse (catalogs/foundation/automation.json@92bb279 considered_not_activated[repository=dependabot/dependabot-core].decision). It proposes pin updates; it does not enforce pinning.
...
Open gaps:
- No retained zizmor negative control shows it rejecting over-broad workflow permissions. The unsafe fixture declares contents: read, and the tests assert only template-injection and unpinned-uses. The scoped-permissions axis rests on configuration and source review (docs/catalog-provenance.md lines 20-24; catalogs/foundation/automation.json@92bb279 considered_not_activated[repository=github/codeql-action].decision), not on native analyzer execution.
- The hosted 0/1/0 attestation qualification is for run 35541322881 at d0fe136c (2026-09-20). On 2026-09-22, publish-catalog.yml added a tag trigger, a changed job condition, a Syft SBOM step and a second attest step (catalogs/foundation/automation.json@92bb279 scheduled_report_only_lanes[source=.github/workflows/publish-catalog.yml]). The sources I read record no hosted run of the current workflow bytes, so neither the archive attestation nor the SBOM attestation is re-qualified.
- The hosted sbom-vuln job in supply-chain.yml has no recorded run ID. Syft's native inventory evidence comes from a local run on one host against one SDK prefix, not whole-host or whole-runtime coverage.
...
- Dependabot's weekly timing and nonempty minor/patch grouping are unobserved. Python and binary pins have no automated updater; the report-only catalog-freshness drift table stands in for one.
- Offline zizmor analysis at the regular persona does not cover online advisories, secrets or provenance (ci-security README lines 48-52).
- No vulnerability threshold policy exists. Grype is report-only, and Trivy is deferred.
