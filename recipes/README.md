...
| `mcp-inspector` · `2.7.0` | `npm install --global --prefix "$STACK_HOME/tools/mcp-inspector-2.7.0" @modelcontextprotocol/inspector@2.7.0` | `mcp-inspector --cli --config "$PROJECT_ROOT/.mcp.json" --server socraticode --method tools/list --format json --stored-auth-only --cwd "$PROJECT_ROOT"`. Handshake/schema discovery only; follow with a selected tool call for functionality. Read the installed version with `npm ls --global --prefix "$STACK_HOME/tools/mcp-inspector-2.7.0" --depth=0`: the launcher has no version flag and forwards unknown arguments to the default web mode, so `mcp-inspector --version` starts the web UI with `--version` as a stdio server command (observed 2026-09-25 on 2.7.0; 2.8.0's launcher is identical). |
