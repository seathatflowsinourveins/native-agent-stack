...
directory stands in for the CLI: it prints scripted `--json` events and writes scripted replies. It is found
through PATH, which holds only its directory, /usr/bin and /bin, and CODEX_HOME points at an empty temporary
directory, so the real Codex is never reached.
"""
import contextlib
import copy
import fcntl
...
args = sys.argv[1:]
if args == ["--version"]:
    print(os.environ.get("FAKE_CODEX_VERSION", "codex-cli 0.157.1"))
    sys.exit(0)
...
with open(os.path.join(config["record_dir"], "%s_%s_%s_%d.json" % (model, effort, accessions[0], number)), "w") as out:
    json.dump({"argv": args, "cwd": os.getcwd(), "rust_log": os.environ.get("RUST_LOG"),
...
if action == "hang":
    ...
    time.sleep(60)
if action == "limit":
    events += [{"type": "error", "message": config["limit_text"]},
...
class Fixture:
    """A synthetic acquisition, a plan frozen for it, a lock directory, a state directory and the fake codex."""
    ...
    def __init__(self, case, rows, plan_changes=None):
        ...
        self.home.mkdir()
        self.configure()
        # Only the fake is reachable: the real codex lives in neither directory, and CODEX_HOME holds no sign-in.
        self.path = f"{self.bin}{os.pathsep}/usr/bin{os.pathsep}/bin"
        case.assertEqual(shutil.which("codex", path=self.path), str(self.codex))
        patcher = mock.patch.dict(os.environ, {"FAKE_CODEX_CONFIG": str(self.config), "CODEX_HOME": str(self.home),
                                               "RUST_LOG": "trace", "PATH": self.path})
        patcher.start()
    ...
    def cli(self, *args, env=None):
        environment = {"PATH": self.path, "FAKE_CODEX_CONFIG": str(self.config),
                       "CODEX_HOME": str(self.home), "HOME": str(self.root), **(env or {})}
    ...
    def test_changed_frozen_values_are_refused(self):
        ...
        for path, value in changes:
            ...
            with self.subTest(path=path), self.assertRaises(ValueError):
                RUN.check_plan(plan)
        plan = copy.deepcopy(PLAN)
        plan["codex"]["argv_after_binary"] = ["--search", *plan["codex"]["argv_after_binary"]]
...
class RunnerTests(unittest.TestCase):
    def test_codex_command_line_directory_stdin_environment_and_private_files(self):
        fixture = Fixture(self, rows_for(20))
        self.assertEqual(fixture.run("S1"), RUN.EXIT_DONE)
        record = json.loads((fixture.records_dir / f"gpt-6-sol_medium_{fixture.first(1)}_1.json").read_text())
        arm_dir = fixture.state / "arms" / "S1"
        argv = record["argv"]
        self.assertEqual(argv[:-4], ["exec", "--ignore-user-config", "--skip-git-repo-check", "-s", "read-only",
                                     "-m", "gpt-6-sol", "-c", 'model_reasoning_effort="medium"', "--output-schema",
                                     str(arm_dir / "schema.json")])
        self.assertEqual(argv[-4:-1], ["-o", str(arm_dir / "batches/b002/attempt-1/reply.json"), "--json"])
        self.assertNotIn("--search", argv)
        self.assertEqual(argv[-1], RUN.build_prompt((HERE / "prompt.txt").read_text(), fixture.batches[1]))
        self.assertEqual(record["cwd"], str(fixture.state / "empty"))
        self.assertTrue(record["stdin_devnull"])
        self.assertIsNone(record["rust_log"])
        self.assertEqual(record["schema"], json.loads((HERE / "response.schema.json").read_text()))
        self.assertEqual(list((fixture.state / "empty").iterdir()), [])
    ...
    def test_usage_limit_stops_the_arm_and_the_batch_resumes_after_the_reset(self):
        ...
        fixture.configure(sleep=0.4, rules=[{"batch": second, "attempt": 1, "action": "limit"},
                                            {"batch": second, "attempt": 2, "action": "exit1"}])
        out = io.StringIO()
        ...
        self.assertEqual((note["arm"], note["batch"], note["attempt"]), ("A0", "b002", 1))
        ...
        self.assertEqual((records[("b002", 1)]["outcome"], records[("b002", 1)]["limit"]), ("limit", True))
    ...
    def test_a_failed_batch_runs_once_more_and_then_never_again(self):
        ...
        fixture.configure(rules=[{"batch": fixture.first(0), "attempt": 1, "action": "exit1"},
                                 {"batch": fixture.first(1), "action": "invalid_reply"}])
        self.assertEqual(fixture.run("L0"), RUN.EXIT_DONE)
    ...
    def test_timeout_kills_the_call_and_counts_as_a_failure(self):
        ...
        records = fixture.records("A0")
        self.assertEqual((records[("b001", 1)]["reason"], records[("b001", 1)]["timed_out"]), ("timeout", True))
        self.assertEqual(records[("b001", 2)]["outcome"], "ok")
        pid = fixture.calls()[0]["pid"]
        with self.assertRaises(ProcessLookupError):
            os.kill(pid, 0)

    def test_an_abandoned_attempt_counts_as_a_failure(self):
        fixture = Fixture(self, rows_for(10))
        attempt = fixture.state / "arms/A0/batches/b001/attempt-1"
        for path in (fixture.state, fixture.state / "arms", fixture.state / "arms/A0",
                     fixture.state / "arms/A0/batches", fixture.state / "arms/A0/batches/b001", attempt):
            path.mkdir(mode=0o700)
        (attempt / "events.jsonl").write_text(json.dumps({"type": "turn.started"}) + "\n")
        self.assertEqual(fixture.run("A0"), RUN.EXIT_DONE)
        records = fixture.records("A0")
        self.assertEqual((records[("b001", 1)]["outcome"], records[("b001", 1)]["reason"]), ("failed", "abandoned"))
        self.assertEqual(records[("b001", 2)]["outcome"], "ok")
    ...
    def test_sigterm_records_the_running_call_as_interrupted_without_counting_it(self):
        ...
        self.assertEqual((fixture.records()[("b001", 1)]["outcome"], fixture.records()[("b001", 1)]["interrupted"]),
                         ("interrupted", True))
        process = fixture.cli(*args)
    ...
    def test_refusals_before_any_call(self):
        ...
        with mock.patch.dict(os.environ, {"FAKE_CODEX_VERSION": "codex-cli 0.158.0"}):
        ...
        (fixture.state / "LIMIT").write_text("{}")
...
class DecisionRuleTests(unittest.TestCase):
    ...
    def test_lowest_billed_routable_arm_wins_and_ties_go_to_wall_time_then_plan_order(self):
        ...
        self.assertEqual(selected["ranked"], ["S0", "A1", "A0"])
        self.assertFalse(criteria["S1"]["noninferior_micro_f1"])
        self.assertTrue(self.decide(base, lowers={"S1": -0.02})[0]["arm"] == "S1")  # the margin is inclusive
        ...
        cheap_default = {arm: summary(1000) for arm in base}
        cheap_default["A0"] = summary(10)
        self.assertEqual(self.decide(cheap_default)[0]["outcome"], "keep_default")

    def test_unknown_usage_is_routable_but_not_ranked(self):
        summaries = {"A0": summary(900, usage_complete=False), "S0": summary(100, usage_complete=False)}
        selected, _ = self.decide(summaries)
        self.assertEqual((selected["arm"], selected["outcome"], selected["routable"]), (None, "no_ranked_arm",
                                                                                       ["A0", "S0"]))
    ...
    def test_billed_tokens_count_reasoning_once(self):
        records = [{"outcome": "ok", "usage": {"input_tokens": 1000, "cached_input_tokens": 400,
                                               "cache_write_input_tokens": 0, "output_tokens": 120,
                                               "reasoning_output_tokens": 100}},
                   {"outcome": "failed", "usage": {"input_tokens": 500, "cached_input_tokens": 0,
                                                   "output_tokens": 30, "reasoning_output_tokens": 30}},
                   {"outcome": "limit", "usage": None}]
        ...
        usage = ANALYZE.usage_summary(records + [{"outcome": "failed", "usage": None}], 10)
        self.assertEqual((usage["calls_without_usage"], usage["usage_complete"]), (1, False))
...
class AnalysisTests(unittest.TestCase):
    def test_analysis_on_synthetic_labels(self):
        ...
        self.assertEqual(decision["selected"]["ranked"], ["S0", "A1", "A0"])
    ...
    def test_incomplete_or_missing_arms_keep_the_decision_provisional(self):
        ...
        self.assertEqual(decision["selected"]["arm"], "A0")
