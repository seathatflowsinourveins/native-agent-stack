...
from core.canon import atomic_write_results, dumps, sha256_file
...
def gate_context(ctx: dict, purpose: str, now: float, retry_of=None) -> dict:
    ...
    validated = gate.validated_items(val["results"]) if val["results"] else []
    _, last = window(ctx, blocks_done(ctx["run_log"]))
    g.update({"validation_present": val["present"], "validation_sha256": val["sha256"],
              "validation_committed_sha256": val["committed_sha256"],
...
def _run_fetch(ctx, purpose, auth, bases, snapshot_root, spec_for, mode, enum_date, transports, clock):
    """The action's fetch, sealed as <root>/<purpose>-<id>. A retry of a failed action whose snapshot was sealed
    reuses that snapshot (run_discipline.once: the same input snapshot). The fetch-incomplete rate is over the
    requests the action's own plan required; a collection batch's failed screen request makes its symbol-sessions
    membership-unknown instead (populations.fetch_failures)."""
    ...
    prior = gate.sequence(ctx["access_log"])["completions"].get(auth.get("retry_of")) if auth.get("retry_of") else None
    if prior is not None and prior.get("snapshot_sha256"):
        d, sha = Path(snapshot_root) / f"{purpose}-{auth['retry_of']}", prior["snapshot_sha256"]
    else:
        live = HoldoutStore(bases, Store())
        driver.stage_fetch(planner, transports, live, enum_date, clock=clock)
        d = Path(snapshot_root) / f"{purpose}-{auth['authorization_id']}"
        sha = live.write(d)
    sealed = HoldoutStore(bases, Store.read(d, sha))
    ...
    return sealed, sha, void_rate(by_kind)
...
def count(ctx: dict, authorization_id: str, snapshot_root, transports, now: float, clock=driver.utc_now) -> dict:
    ...
    start, status, sha, digest, ext, void = iso_utc(now), "failed", None, None, None, None
    try:
        spec_for = _spec_factory(ctx, n0, last, late, exposed, None)
        sealed, sha, rate = _run_fetch(ctx, "count", auth, bases, snapshot_root, spec_for, "count", enum_date,
                                       transports, clock)
        ...
        _finish(ctx, authorization_id, start, now, status, sha, 0, digest, [],
                {"purpose": "count", "sessions": [n0, last], "block": blocks, "extension": ext, "void": void,
                 "enumeration_fetch_date": enum_date}, [sha] if sha else [])
    return {"results_sha256": digest, "extension": ext, "window": [n0, last]}


def not_read_results(ctx: dict, carried, reason: str) -> dict:
    """chronology.holdout.read_timing and outcome_reporting.rule: the fixed label for every carried item."""
    labels = {i: ("not supported (holdout not read)" if i in carried else "not carried") for i in ITEM_IDS}
    return {"kind": "mover_v3_holdout_read", "stage": "holdout", "read": False, "reason": reason,
            "carried": list(carried), "labels": labels, "verdicts": ST.hypothesis_verdict("holdout", labels),
            "protocol_sha256": ctx["protocol_sha256"], "study_tree": ctx["tree"],
...
def read(ctx: dict, authorization_id: str, snapshot_root, transports, now: float, clock=driver.utc_now,
    ...
    refused, a count or the read's own fetch is void, or the deadline has passed."""
    ...
    if no_final and now < read_deadline(ctx, last):
    ...
    elif now >= read_deadline(ctx, last):
    ...
    try:
        ...
        sealed, sha, rate = _run_fetch(ctx, "read", auth, bases, snapshot_root, spec_for, "plan",
                                       enum_date or iso_utc(now)[:10], transports, clock)
        if rate["void"]:
        ...
        _finish(ctx, authorization_id, start, now, status, sha, 0, digest, [],
                {"purpose": "read", "sessions": [n0, last]}, [sha] if sha else [])
