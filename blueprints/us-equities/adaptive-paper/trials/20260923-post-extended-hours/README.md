# Adaptive paper after-hours trial, 2026-09-23 19:01 ET: first extended-hours run, stopped by the data timeout
...
Config: `config-post-liquid-9967f44d.json` (sha256 `9967f44d5ec2…`). Compared with the durable ledger's
`77244c39` config:
- the numeric limits are the same: 1x, 1-share orders, 25 USD loss and drawdown, 3 s quote age, 15 bps spread;
...
- Preflight `ready`: flat, no open orders, config sha matched.
...
- `scheduled-trial.log` shows `no data received on data stream for 3.0s, reconnecting`. That line comes from alpaca-py's
  data websocket (`alpaca/data/live/websocket.py`). `transport.py` passes `data_timeout=quote_timeout`, which is the
  config's 3 s quote age, so a 3 s pause across all four subscribed symbols made alpaca-py reconnect, and the runner
  ended the run on the connection gap.
