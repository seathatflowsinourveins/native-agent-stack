...
The [#5039 probe](probes/repro_5039.py) (2026-09-24, local integration on the installed wheel) runs a synthetic EUR/USD `BacktestEngine` whose strategy raises inside `on_order_filled`. On 2.0.0rc5 it prints `upstream_loss_reproduced True` and exits 0: the handler raised, `run()` raised nothing and 49 later bars were still processed ([nautilus_trader#5039](https://github.com/nautechsystems/nautilus_trader/issues/5039)). It exits 1 once a release surfaces the exception or stops, which is the overturn signal for the adaptive-paper callback guard (E3; the `LiveNode` counterpart is `UpstreamCallbackLoss` in `tests/test_adaptive_paper_native.py`). Run it with `rtk proxy "$NT" blueprints/us-equities/engine-nautilus/probes/repro_5039.py`.
