...
class OrderBudget:
    ...
    def __init__(self, max_orders: int, max_quantity: int):
    ...
    def reserve(self, case: str, qty) -> int:
        qty = _dec(qty)
...
class RunContext:
    ...
    def __init__(self, plan: dict, prefix: str, server_minus_local_s: float = 0.0):
        ...
        b = plan["bounds"]
        self.budget = OrderBudget(b["max_orders"], b["max_quantity_per_order"])
...
def build_strategy_class():
    ...
    class PaperOrderStrategy(Strategy):
        ...
        def _submit(self, case, suffix, side, price_dec, qty_int):
            ...
            n = self.ctx.budget.reserve(case, qty_int)  # raises BudgetError before any order exists
            instrument = self.cache.instrument(self._iid)
...
def provisional_receipt(receipt: dict, ctx: RunContext) -> dict:
    ...
    out.update(status="cleanup_required", evidence_class="native_paper", provisional=True, note=PROVISIONAL_NOTE,
               run={"run_prefix": ctx.prefix}, flat_proof={"status": "not_attempted"})
    return out
...
def cmd_run(a, check_fn=run_check, node_fn=run_node, now_fn=None, sleep_fn=time.sleep) -> int:
    t0 = time.monotonic()
