...
def main():
    ...
    parser.add_argument('--reparse', type=Path,
                        help='recompute the parsed fields of an existing receipt from its retained log')
    ...
    if args.reparse:
        ...
        args.reparse.write_text(json.dumps(receipt, indent=2) + '\n')
        return
    ...
    (args.out / f'{args.label}.receipt.json').write_text(json.dumps(receipt, indent=2) + '\n')
