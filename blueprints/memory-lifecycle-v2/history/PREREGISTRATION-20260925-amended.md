...
That is 16 phase-"post" calls: 14 read-only persistence checks, plus 2 calls for the
write-capability bonus (a write, then an immediate read-back query). (Corrected from an
initial miscount of 15 -- the table row above always specified both the write AND the
read-back as the bonus check; only the arithmetic total in this sentence was wrong. Caught by
`exercise.py`'s own `len(outcomes) != N` guard failing closed on the FIRST run attempt for
2.3.2, before that attempt produced a complete result; no check, pass bar, or fixture content
changed. That attempt's partial output was discarded and phase "pre" and phase "post" were
both re-run clean for 2.3.2. See `results-20260925.json` for the corrected preregistration
sha256 recorded after this fix, alongside the original.)
...
"pre" tool-call count not being exactly 69, or phase "post" not exactly 16. A failing check
