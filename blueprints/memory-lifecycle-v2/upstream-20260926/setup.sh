...
# (rustup-init from $S/dl), pinned-tag shallow clones, the toolchain
# rust-toolchain.toml pins, and `cargo fetch --locked`. No record of a
# rustup-init checksum check before this script ran was kept. The binary it
# ran was compared with its published sha256 after the runs
# (verify_rustup_init.py, output in rustup-init-verification.json).
set -euo pipefail
S="$(cd "$(dirname "$0")" && pwd)"
