...
hf_fields() {  # hf_fields <repo> <revision> <file> <field...>: print selected JSON fields of a small file
  ...
  curl -fsS "https://huggingface.co/$repo/resolve/$rev/$file" -o "$work/hf.json" || { report pass fail "hf $repo/$file" "download failed"; return; }
  fields=$(python3 -c 'import json,sys
d=json.load(open(sys.argv[1]))
print(" ".join(f"{k}={json.dumps(d.get(k))}" for k in sys.argv[2:]))' "$work/hf.json" "$@")
  report pass pass "hf $repo/$file" "at $rev (sha256 $(sha256sum "$work/hf.json" | cut -d' ' -f1)): $fields"
}
