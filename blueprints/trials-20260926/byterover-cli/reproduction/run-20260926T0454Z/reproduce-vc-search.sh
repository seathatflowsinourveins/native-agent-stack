...
# directory), disables the bundled @oclif/plugin-update background autoupdate
# (BRV_DISABLE_AUTOUPDATE=1), commits two notes to a project's context tree with `brv vc`,
# runs `brv search --format json` for a positive, a second positive and a negative query,
# and checks every retained output with check_search.py, including two deliberately wrong
# expectations that must fail. `brv restart` then stops the daemon it spawned and the
# workspace is removed. No LLM, network model endpoint or cloud account is used, and no
# ByteRover state is written outside the workspace; npm's shared download cache is used for
# the package download. Only <output-dir> receives files. The shell/Python checks are local
# integration glue; the brv outputs are the tool's own.
set -u
