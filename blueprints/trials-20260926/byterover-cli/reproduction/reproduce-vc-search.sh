...
#
# Processes: cleanup signals only processes this script started. After every command,
# owned_processes.py records the daemon named by the workspace's own daemon.json (one of
# its arguments must be a brv-server.js under the workspace prefix) and that daemon's
# descendants, each with its start time. On exit it sends SIGTERM to the recorded daemon,
# which stops its agents, and SIGKILL to any recorded process still running 10 s later; the
# script exits 3 if one outlives that. No other process is looked at or signalled.
#
...
# <output-dir> receives files. The shell/Python checks are local integration glue; the brv
# outputs are the tool's own.
set -u
...
record_processes() {  # the daemon this workspace's own daemon.json names, and its descendants
  ...
  python3 "$here/owned_processes.py" record "$ledger" "$daemon_file" "$work/prefix/" \
    >> "$out/80-owned-processes.stdout" 2>> "$out/80-owned-processes.stderr"
}
# shellcheck disable=SC2329  # invoked by the EXIT trap
cleanup() {  # exits 3 when a recorded process outlived the stop
  cleanup_status=0
  record_processes
  if [ -f "$ledger" ]; then
