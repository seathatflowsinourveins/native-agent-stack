...
#
# This is the fixed copy of ../connect-local-llm-safe.sh, which stays byte-identical as the
# version that credential-argv-probe-20260926T111651Z.txt checked. That version cleared the key
# variable with `KEY=""`, which keeps an export attribute KEY inherited from the caller's
# environment, so `read` then put the key into curl's environment. This copy turns allexport
# off (bash, which is /bin/sh on some hosts, imports it from an exported SHELLOPTS) and unsets
# KEY before reading it. KEY is the only variable that ever holds the key.
#
...
#
# Step 1, reachability, is automatic. The shell's `read` builtin loads the key into an
# unexported shell variable, and curl gets the Authorization header on standard input
# (`-H @-`, curl 7.55.0 and later). `printf` is a shell builtin in sh and bash, so the key
# never enters an argument list or an environment. The only thing on a command line is the
# key file's path.
#
...
set -eu
# Nothing assigned below may be exported: allexport would export KEY on assignment.
set +a
...
# `unset` drops an export attribute KEY inherited from the caller; `KEY=""` alone keeps it.
unset KEY
KEY=""
