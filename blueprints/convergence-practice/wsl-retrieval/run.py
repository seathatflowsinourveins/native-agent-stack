...
def record_command(label, argv, run, env, path_scopes, facts, result):
    """Retain argv boundaries and cwd, replacing private path roots with scope markers."""
    argv = [str(value) for value in argv]

    def sanitize(value):
        for path, marker in sorted(path_scopes.items(), key=lambda item: len(str(item[0])), reverse=True):
            value = re.sub(re.escape(str(path)) + r'(?=$|/)', lambda _: marker, value)
        return value

    fact = {'label': label, 'argv': [sanitize(value) for value in argv],
            'cwd': sanitize(str(run)), 'exit_code': None}
    facts.append(fact)
    save(run/'receipt.partial.json', result)
    stdout, stderr = '', ''
    try:
        out = subprocess.run(argv, cwd=run, env=env, text=True,
                             capture_output=True, timeout=30, check=False)
        stdout, stderr = out.stdout, out.stderr
        fact['exit_code'] = out.returncode
        return out, fact
        ...
        fact['failure'] = 'launch_error'
        raise
    finally:
        for suffix, value in [('stdout', stdout), ('stderr', stderr)]:
            data = value if isinstance(value, bytes) else value.encode()
            (run/(label+'.'+suffix)).write_bytes(data)
            fact[suffix+'_bytes'] = len(data)
            fact[suffix+'_sha256'] = sha(run/(label+'.'+suffix))
        save(run/'receipt.partial.json', result)
...
def main():
    ...
    path_scopes = {str(run): '<RUN>', str(Path.home()): '<HOME>'}
    for name in ['source', 'rg', 'ast_grep', 'node', 'package_prefix']:
        path = getattr(args, name)
        if path is not None:
            path_scopes[str(path)] = '<' + name.upper() + '>'

    def command(label, argv, extra_env=None):
        return record_command(label, argv, run, env | (extra_env or {}), path_scopes, facts, result)
