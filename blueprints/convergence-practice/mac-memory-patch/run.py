#!/usr/bin/env python3
"""Mac-only, synthetic ai-memory 2.3.1 -> 2.3.2 qualification; no promotion."""
import argparse
import datetime
import hashlib
import json
...
def main():
    ...
    frozen.mkdir()
    for filename in ['run.py', 'pins.json', 'oracle.json', 'config.toml']:
    ...
    try:
        ...
        profile = attempt / 'sandbox.sb'
        profile.write_text('(version 1)\n(deny default)\n(allow process*)\n(allow sysctl-read)\n'
            '(allow mach-lookup)\n(allow file-read* '
            + ' '.join('(subpath '+json.dumps(p)+')' for p in ['/System', '/usr', '/Library', '/private/etc', '/dev', '/private/var/db', str(prefix), str(old.parent)])
            + ')\n(allow file-write* (subpath '+json.dumps(str(attempt))+'))\n')
        sandbox = ['/usr/bin/sandbox-exec', '-f', str(profile)]
        probe = ('import errno,pathlib,socket; s=socket.socket(); '
                 '\ntry: s.connect(("127.0.0.1",9))\nexcept OSError as e: assert e.errno==errno.EPERM\nelse: raise AssertionError("network allowed")\n'
                 'try: pathlib.Path('+repr(str(prefix/'escape-probe'))+').write_text("guard failure")\n'
                 'except OSError as e: assert e.errno==errno.EPERM\nelse: raise AssertionError("outside write allowed")\nprint("guards passed")')
        cp = command('sandbox-guards', sandbox+['/usr/bin/python3', '-c', probe], env)
        check('sandbox_network_and_write_guards', cp.returncode == 0 and cp.stdout.strip() == 'guards passed')
        versions = {}
