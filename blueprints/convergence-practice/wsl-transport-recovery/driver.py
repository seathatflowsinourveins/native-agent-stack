...
def main():
    ...
    files={name:(HERE/name).read_bytes() for name in ['plan.json','remote.py','driver.py']}
...
'''
    ...
    try:
        ...
        with (work/'job-transport.stdout').open('x') as out,(work/'job-transport.stderr').open('x') as err:
            ...
            if code!=-15:raise ValueError('expected owned SSH client SIGTERM exit')
            ...
            if after['status']=='running': retry=False
            elif after['status'] in ['aborted','failed']: retry=True
            else:raise ValueError('unsupported native reconnect status')
            remote('release','release',*(['--retry'] if retry else []))
            native=remote('finish','finish')
            if not native['passed']:raise ValueError('native final acceptance failed')
        result.update(passed=True,native=native,transport={'active_before_signal':True,
            ...
            'real_network_outage_or_host_restart':False},ssh_command_exit_codes=codes)
        success=True
