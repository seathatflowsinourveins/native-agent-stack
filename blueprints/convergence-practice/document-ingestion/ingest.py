...
def run(executable, output, root=HERE):
    ...
    receipt={'kind':'historical_native_document_qualification','evidence_class':'native_cli_execution','attempt':1,'status':'passed' if all(checks.values()) else 'failed','version':version_line,'executable_sha256':digest(executable),'commands':commands,'checks':checks,'pages':len(pages),'data_cells':sum(len(row) for p in pages for t in p['tables'] for row in t['rows']),'query_count':len(answers),'positive_queries':8,'negative_queries':4,'elapsed_seconds':round(time.monotonic()-started,4),'native_invalid_input':{'exit_code':refusal.returncode,'stdout_bytes':len(refusal.stdout),'stderr':refusal.stderr.decode('utf-8','replace')},'usage':{'model_calls':0,'embedding_calls':0,'provider_tokens':0,'scope':'This deterministic runner only; coordinator/research token usage excluded and unknown.'},'artifacts':{n:digest(output/n) for n in ('extraction.json','retrieval.json','lumen.xhtml','w3c.xhtml')},'freeze_sha256':digest(root/'freeze.json')}
...
if __name__ == '__main__':
    ...
    args=p.parse_args();r=run(args.pdftotext.resolve(),args.output.resolve());print(json.dumps({'status':r['status'],'checks':r['checks']}));raise SystemExit(0 if r['status']=='passed' else 1)
