...
def private_directory(value, source=HERE):
    """Respect the containing checkout, including Git worktrees and copied sources."""
    source = Path(source).resolve()
    result = subprocess.run(["git", "-C", str(source), "rev-parse", "--show-toplevel"],
                            capture_output=True, text=True, check=False)
    boundary = Path(result.stdout.strip()).resolve() if result.returncode == 0 else source
    destination = Path(value).resolve()
    if destination == boundary or boundary in destination.parents:
        raise ValueError("private_run_directory_must_be_outside_source_repository")
    return destination
...
def cleanup_owned(clients, marker, workspace, cleanup):
    """A cleanup race must remain evidence, never suppress the attempt receipt."""
    ...
    for client in clients:
        try:
            ...
            cleanup["errors"].append({"scope": client.label, "type": type(exc).__name__})
    try:
        if marker is not None and wait_alive(marker):
            cleanup["final_wait_cleanup_needed"] = stop_wait(marker, workspace)
        cleanup["wait_stopped"] = marker is not None and not wait_alive(marker)
    except Exception as exc:
        cleanup["wait_stopped"] = False
        cleanup["errors"].append({"scope": "owned_wait", "type": type(exc).__name__})


def run(args):
    root = private_directory(args.run_dir)
    ...
    try:
        ...
        cleanup_owned(clients, marker, workspace, cleanup)
    checks["owned_cli_processes_stopped"] = bool(clients) and all(c.proc.poll() is not None for c in clients)
    checks["owned_wait_stopped"] = cleanup.get("wait_stopped", False)
