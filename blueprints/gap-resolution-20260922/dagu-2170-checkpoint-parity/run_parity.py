#!/usr/bin/env python3
"""Run the SAME checkpoint/stop/retry fixture used by
blueprints/convergence-practice/job-recovery/run.py (accepted for Dagu 2.16.6)
against an arbitrary Dagu binary, so the outcome can be diffed across versions.

This is a version-parameterized copy of that runner: identical DAG shape,
identical fixture.py (imported unmodified from the job-recovery directory,
never edited), identical stop/retry/checkpoint-reuse assertions. The only
difference from the original is that the hardcoded '2.16.6 required' guard
is replaced with an --expect-version check so 2.17.0 can be exercised too.
"""
import argparse
import json
import os
...
ROOT = HERE.parents[2]
JOB_RECOVERY_DIR = ROOT / "blueprints/convergence-practice/job-recovery"
sys.path.insert(0, str(JOB_RECOVERY_DIR))
...
def run(dagu, work, expect_version):
    ...
    shutil.copyfile(JOB_RECOVERY_DIR / "fixture.py", work / "fixture.py")
    ...
    frozen = {"frozen_at_utc": datetime.now(timezone.utc).isoformat(),
              "repository_inputs": {str(path.relative_to(ROOT)): fixture.digest(path) for path in
                                     [JOB_RECOVERY_DIR / "plan.json", JOB_RECOVERY_DIR / "run.py",
                                      JOB_RECOVERY_DIR / "fixture.py",
    ...
    if expect_version not in version.split():
        raise ValueError(f"This fixture run was invoked expecting Dagu {expect_version}, got {version!r}")
    ...
    try:
        ...
        call("retry", base + ["retry", "--run-id", run_id, str(workflow)])
        ...
        result = {"status": "passed", "platform": platform.system() + " " + platform.release(),
                  "architecture": platform.machine(), "dagu_version": version, "python_version": platform.python_version(),
...
if __name__ == "__main__":
    ...
    print(json.dumps(run(args.dagu, args.work, args.expect_version), indent=2))
