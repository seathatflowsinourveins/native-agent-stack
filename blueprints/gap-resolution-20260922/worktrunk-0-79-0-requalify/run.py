...
def pinned_binaries(prefix, pins, installed):
    expected = {tool['name']: tool for tool in pins['components']}
    actual = {tool['name']: tool for tool in installed['components']}
    if (len(expected) != len(pins['components']) or len(actual) != len(installed['components'])
            or set(actual) != set(expected)):
        raise ValueError('Installed component identities differ from frozen pins')
    binaries = {}
    for name, pin in expected.items():
        tool = actual[name]
        required = {'version': pin['version'], 'source_commit': pin['source_commit'],
                    'archive_sha256': pin['archive']['sha256'], 'license': pin['license']}
        if any(tool.get(key) != value for key, value in required.items()):
            raise ValueError('Installed release identity differs from frozen pins')
        archive = prefix / 'downloads' / pin['archive']['name']
        if not archive.resolve().is_relative_to(prefix) or sha(archive.read_bytes()) != pin['archive']['sha256']:
            raise ValueError('Retained release archive escaped prefix or changed')
        with tarfile.open(archive) as release:
            members = [member for member in release.getmembers()
                       if member.isfile() and Path(member.name).name == pin['executable']]
            if len(members) != 1:
                raise ValueError('Pinned release lacks exactly one native executable')
            archive_binary_sha = sha(release.extractfile(members[0]).read())
        binary = prefix / tool['binary']
        if (not binary.resolve().is_relative_to(prefix)
                or sha(binary.read_bytes()) != archive_binary_sha
                or tool['binary_sha256'] != archive_binary_sha):
            raise ValueError('Installed binary escaped prefix or changed from pinned release')
        binaries[name] = binary
    return binaries


def verify_versions(versions, pins):
    for tool in pins['components']:
        expected = tool['version'] if tool['name'] == 'gitleaks' else 'wt v' + tool['version']
        if versions.get(tool['name']) != expected:
            raise ValueError('Native version differs from frozen pins')


def redact(data):
    """Remove the sole generated inert secret from every retained native stream."""
    if isinstance(data, str):
        data = data.encode()
    return (data or b'').replace(inert_token().encode(), b'REDACTED')
...
def qualification(prefix, work):
    ...
    def invoke(name, args, cwd=work, expected=0):
        ...
        try:
            ...
            stdout, stderr, exit_code = error.stdout, error.stderr, None
            timed_out = True
        out = logs / (name + '.stdout'); err = logs / (name + '.stderr')
        out.write_bytes(redact(stdout) if timed_out else stdout)
        err.write_bytes(redact(stderr) if timed_out else stderr)
        public = [str(x).replace(str(work), '$RUN').replace(str(prefix), '$PREFIX') for x in args]
        journal.append({'name': name, 'argv': public,
                        'cwd': str(cwd).replace(str(work), '$RUN'), 'exit_code': exit_code,
                        'timed_out': timed_out,
                        'elapsed_seconds': round(time.monotonic() - started, 6),
                        'stdout_sha256': sha(out.read_bytes()), 'stderr_sha256': sha(err.read_bytes())})
        write_json(work / 'receipt.json', receipt)
        if timed_out:
            raise ValueError(f'{name}: timed out; partial redacted output retained in owned logs')
        if exit_code != expected:
            raise ValueError(f'{name}: exit {exit_code}, expected {expected}; inspect owned logs')
        return stdout.decode()

    try:
        installed = json.loads((prefix / 'installation.json').read_text())
        pins = json.loads((HERE / 'pins.json').read_text())
        binaries = pinned_binaries(prefix, pins, installed)
        ...
        verify_versions(receipt['versions'], pins)
        token = inert_token()
